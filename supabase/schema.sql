...
begin
  ...
  if new.email <> old.email and not public.is_admin() then new.email = old.email; end if;
  return new;
end $$;
...
begin
  if public.is_admin() then
    if new.assigned_to is distinct from old.assigned_to then new.assigned_at = case when new.assigned_to is null then null else now() end; end if;
  elsif public.is_team() and old.assigned_to = auth.uid() then
    -- team member: everything except status / completed_at stays as it was
    new := old;
    new.status := (select status from (values (1)) as v cross join lateral (select new.status) s);
  else
...
-- (the odd status line above keeps the caller's new status while resetting every other column)
...
begin
  ...
  if auth.uid() is not null then new.customer_id = auth.uid(); end if;
  return new;
end $$;
