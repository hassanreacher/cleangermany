...
import { ArrowRight, MessageCircle, Star, CalendarCheck, ShieldCheck, Sparkles, ChevronDown, BadgePercent } from 'lucide-react'
import { Magnetic, TextReveal, ease } from '@/components/motion'
import { bookedSlots, reviewStats } from '@/lib/orders'
import { supabaseConfigured } from '@/lib/supabase'
import { nextAvailable, todayISO, addDays } from '@/lib/slots'
...
export function Hero() {
  ...
  const [next, setNext] = useState<{ date: string; times: string[] } | null | undefined>(undefined)
  const [stats, setStats] = useState<{ avg: number; count: number } | null>(null)

  useEffect(() => {
    if (!supabaseConfigured) { setNext(nextAvailable(new Set(), 1)[0] ?? null); return }
    const t = todayISO()
    bookedSlots(t, addDays(t, 45)).then(b => setNext(nextAvailable(b, 1)[0] ?? null)).catch(() => setNext(nextAvailable(new Set(), 1)[0] ?? null))
    reviewStats().then(setStats).catch(() => setStats(null))
  }, [])
      ...
      <div className="container-x grid lg:grid-cols-[1.15fr_1fr] gap-12 lg:gap-8 items-center">
        <div className="relative z-10">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.45, ease }} className="mt-6 max-w-xl text-lg text-muted leading-relaxed">
            Büro, Praxis, Kita, Schule, Treppenhaus, Gewerbe oder Zuhause – wir bringen alles zum Glänzen. Angebot in <b className="text-text">2 Minuten</b> online anfragen oder im Chat mit <b className="text-text">Clea</b>. Kostenlose Besichtigung, schriftliches Angebot.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9, duration: 0.8 }} className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 text-sm text-muted">
            {stats && <span className="inline-flex items-center gap-1.5"><span className="flex text-amber-400">{[...Array(5)].map((_, i) => <Star key={i} size={14} fill={i < Math.round(stats.avg) ? 'currentColor' : 'none'} />)}</span><b className="text-text">{stats.avg.toLocaleString('de-DE')}</b> · {stats.count} Bewertung{stats.count === 1 ? '' : 'en'}</span>}
            ...
            <span className="inline-flex items-center gap-1.5"><BadgePercent size={16} className="text-cyan-deep" /> {business.directDiscount} % bei Direktkontakt</span>
          </motion.div>
        </div>

        <div className="relative h-[440px] sm:h-[520px] lg:h-[600px]">
          <motion.div style={{ x: sx, y: sy }} className="absolute inset-0">
            <motion.div style={{ y: y1 }} initial={{ opacity: 0, scale: 0.9, rotate: -6 }} animate={{ opacity: 1, scale: 1, rotate: -4 }} transition={{ duration: 1, delay: 0.3, ease }} className="absolute left-[4%] top-[8%] w-[78%] sm:w-[72%] aspect-[4/5] rounded-[32px] overflow-hidden shadow-2xl"><RoomIllustration /></motion.div>
            <motion.div style={{ y: y2 }} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.7, ease }} className="absolute end-0 top-[14%] glass p-4 w-[240px]">
              ...
              {next === undefined ? <div className="mt-2 skeleton h-10 rounded-lg" /> : next ? <div className="mt-2 font-display font-bold text-lg leading-tight">{weekdaysLong[new Date(next.date + 'T00:00').getDay()].slice(0, 2)}, {formatDateDE(next.date)}<br /><span className="text-cyan-deep">{next.times[0]} Uhr</span></div> : <div className="mt-2 text-sm">Auf Anfrage</div>}
              <Link to="/termin" className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-cyan-deep">Jetzt anfragen <ArrowRight size={12} className="rtl:rotate-180" /></Link>
            </motion.div>
            <motion.div style={{ y: y1 }} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.9, ease }} className="absolute end-[4%] bottom-[8%] glass p-4 w-[250px]">
              <div className="flex items-center gap-3">
                ...
                <div className="text-sm"><b className="font-display">Clea</b> <span className="text-muted">· KI-Assistentin</span><div className="text-muted text-xs mt-0.5">„Hallo! Was darf ich für Sie anfragen?“</div></div>
              </div>
            ...
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} className="absolute start-0 bottom-[16%] glass px-4 py-3 text-sm font-semibold flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" /> Berlin · {business.hours}
            </motion.div>
          </motion.div>
        </div>
      </div>
  ...
  return (
    <div className="relative w-full h-full" style={{ background: 'linear-gradient(160deg, #0e3b47, #0b2b33)' }}>
      <svg viewBox="0 0 400 500" className="w-full h-full" aria-hidden>
        ...
        <rect width="400" height="340" fill="url(#wall)" /><rect y="340" width="400" height="160" fill="url(#floor)" />
        <rect x="230" y="60" width="130" height="150" rx="10" fill="#fff" opacity=".9" /><rect x="238" y="68" width="114" height="134" rx="6" fill="#8fdcef" /><path d="M295 68 v134 M238 135 h114" stroke="#fff" strokeWidth="4" />
        <rect x="40" y="250" width="44" height="52" rx="8" fill="#0aa3c7" /><path d="M62 250 c-30 -30 -30 -70 0 -80 c30 10 30 50 0 80 M62 250 c-40 -10 -45 -45 -30 -55 c20 5 30 30 30 55 M62 250 c40 -10 45 -45 30 -55 c-20 5 -30 30 -30 55" fill="#1fc0e4" />
        <rect x="110" y="250" width="200" height="90" rx="18" fill="#0e3b47" /><rect x="120" y="230" width="60" height="50" rx="12" fill="#1fc0e4" /><rect x="185" y="230" width="60" height="50" rx="12" fill="#b9eef9" /><rect x="250" y="230" width="50" height="50" rx="12" fill="#1fc0e4" /><rect x="100" y="300" width="220" height="24" rx="12" fill="#0b2b33" />
        <ellipse cx="200" cy="400" rx="140" ry="40" fill="#b9eef9" opacity=".8" /><ellipse cx="200" cy="400" rx="110" ry="28" fill="none" stroke="#fff" strokeWidth="3" />
